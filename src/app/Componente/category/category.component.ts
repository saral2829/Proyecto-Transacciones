...
export class CategoryComponent {
   ...
   getTransactionsByMonth(month: string, year: string): Array<Transaction> {
      let transactions = this.transactions.filter((transaction: any) => {
         return (
   ...
   getGroupsByCategory(transactions: Array<Transaction>, type: string) {
      const groups: any = transactions.reduce(
         (groups: any, transaction: Transaction) => {
            let category: any = this.categories_json.find(
               (category: any) => category.id === transaction.category_id
            );
   ...
   getSumsByCategory(
      ...
      const subtotals = Object.keys(groups).map((category) => {
         ...
         let category_: any = this.categories_json.find(
            (category_: any) => category_.name === category
         );
         return {
            name: category_.name,
            icon: category_.icon,
